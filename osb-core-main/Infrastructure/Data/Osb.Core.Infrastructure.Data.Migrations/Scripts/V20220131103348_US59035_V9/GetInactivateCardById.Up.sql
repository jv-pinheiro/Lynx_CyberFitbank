DROP FUNCTION public.getinactivatecardbyid(bigint);

CREATE OR REPLACE FUNCTION public.getinactivatecardbyid(
	"paramId" bigint)
    RETURNS TABLE("IdentifierCardId" bigint, "AccountId" bigint, "IdentifierCard" character varying, "Pin" character varying, "Salt" character varying, "ReasonCode" integer, "Status" integer, "CreationDate" timestamp without time zone, "UpdateDate" timestamp without time zone, "DeletionDate" timestamp without time zone, "CreationUserId" bigint, "UpdateUserId" bigint) 
    LANGUAGE 'sql'
    COST 100
