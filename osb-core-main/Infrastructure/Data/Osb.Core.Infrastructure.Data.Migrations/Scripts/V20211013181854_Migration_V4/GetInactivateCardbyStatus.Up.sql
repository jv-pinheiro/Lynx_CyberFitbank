CREATE OR REPLACE FUNCTION public.getinactivatecardbystatus(
	"paramStatus" integer)
    RETURNS TABLE("InactivateCardId" bigint, "AccountId" bigint, "OperationId" bigint, "IdentifierCard" character varying, "Pin" character varying, "Salt" character varying, "ReasonCode" integer, "Status" integer, "Attempts" integer, "CreationDate" timestamp without time zone, "UpdateDate" timestamp without time zone, "DeletionDate" timestamp without time zone, "CreationUserId" bigint, "UpdateUserId" bigint) 
    LANGUAGE 'sql'
    COST 100
